...
export async function createTransactionAction(
  ...
  try {
    ...
    return failure(error, idempotencyKey, values, "Não foi possível registrar a movimentação.");
  }
}
...
export async function createTransferAction(
  ...
  try {
    ...
    return failure(error, idempotencyKey, values, "Não foi possível registrar a transferência.");
  }
}
...
function invalid(
  ...
  for (const issue of issues) {
    ...
    fieldErrors[field] = [
      issue.code === "custom" ? `${issue.message}.` : (FIELD_MESSAGES[field] ?? "Valor inválido."),
    ];
  }
...
function failure(
  ...
  if (isAuthFailure(error)) {
    return {
      ...
      values,
      reauth: true,
      message: transactionsErrorMessage(error, "Sua sessão expirou. Entre novamente."),
    };
  }
...
export async function categorizeTransactionAction(
  ...
  const token = await accessTokenOrNull();
  if (!token) {
    return { status: "error", reauth: true, message: "Sua sessão expirou. Entre novamente para continuar." };
  }
  ...
  if (error instanceof ProblemDetailsError) {
    switch (error.code) {
      case PROBLEM_CODES.categoryArchived:
        ...
        return {
          status: "error",
          message: "Esta categoria foi arquivada e não pode mais ser atribuída. A lista foi atualizada; escolha outra.",
        };
