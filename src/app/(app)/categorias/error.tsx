...
export default function CategoriasError({ reset }: { reset: () => void }) {
    ...
    <EmptyState
      ...
      action={
        <span style={{ display: "inline-flex", flexWrap: "wrap", gap: 8, justifyContent: "center" }}>
          ...
          <a className={`${ui.button} ${ui.secondary}`} href="/auth/login?returnTo=/categorias">
            Entrar novamente
          </a>
        </span>
      }
    >
      Pode ser uma falha momentânea de conexão. Suas categorias não foram alteradas.
    </EmptyState>
  );
}
