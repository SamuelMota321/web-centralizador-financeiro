...
export async function createCategoryAction(
  ...
  try {
    ...
    return formFailure(error, name, "Não foi possível criar a categoria.");
  }
...
export async function renameCategoryAction(
  ...
  try {
    ...
    if (!isCategoryUnavailable(error)) {
      return formFailure(error, name, "Não foi possível renomear a categoria.");
    }
...
export async function archiveCategoryAction(categoryId: string): Promise<ArchiveCategoryState> {
  ...
  try {
    ...
    if (!isCategoryUnavailable(error)) {
      return {
        status: "error",
        message: transactionsErrorMessage(error, "Não foi possível arquivar a categoria. Tente de novo."),
      };
    }
    notice = "categoria-indisponivel";
  }
...
function invalidName(name: string, message: string | undefined): CategoryFormState {
  return { status: "invalid", name, message: `${message ?? "Informe um nome válido"}.` };
}
