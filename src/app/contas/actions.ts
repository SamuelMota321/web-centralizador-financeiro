...
export async function createAccountAction(
  ...
  try {
    ...
    return serverFailure(error, "Nao foi possivel criar a conta.");
  }
}
...
export async function updateAccountAction(
  ...
  if (!id.success || !source.success || source.data.id !== id.data) {
    return { status: "error", message: "Nao foi possivel salvar a conta." };
  }
  ...
  try {
    ...
    if (!isAccountUnavailable(error)) {
      return serverFailure(error, "Nao foi possivel salvar a conta.");
    }
...
export async function deactivateAccountAction(
  ...
  const id = accountIdSchema.safeParse(accountId);
  if (!id.success) {
    return { status: "error", message: "Nao foi possivel desativar a conta." };
  }
  ...
  try {
    ...
    if (!isAccountUnavailable(error)) {
      return {
        status: "error",
        message: accountErrorMessage(error, "Nao foi possivel desativar a conta."),
      };
    }
