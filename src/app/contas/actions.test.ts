...
describe("updateAccountAction", () => {
  ...
  it("repassa a confirmacao de duplicidade junto com o diff", async () => {
  ...
  it("devolve as candidatas quando ha possivel duplicidade", async () => {
    const candidates = [
  ...
  it("traduz erros de campo do backend", async () => {
    ...
    expect(state).toEqual({
      status: "invalid",
      fieldErrors: { initialBalance: ["Informe saldo inicial e data de referencia juntos."] },
    });
  });
  ...
  it("informa conta conectada somente leitura em pt-BR", async () => {
    ...
    expect(state).toEqual({
      status: "error",
      message: "Contas conectadas nao podem ser editadas aqui. Voce ainda pode desativa-las.",
    });
  });
});

describe("deactivateAccountAction", () => {
  it("redireciona com aviso de desativacao", async () => {
  ...
  it("trata conta inexistente como indisponivel", async () => {
  ...
  it("mantem o painel com mensagem em pt-BR em falha interna", async () => {
    ...
    expect(await deactivateAccountAction(ACCOUNT_ID)).toEqual({
      status: "error",
      message: "Nao foi possivel desativar a conta.",
    });
  });

  it("recusa id invalido sem chamar a API", async () => {
