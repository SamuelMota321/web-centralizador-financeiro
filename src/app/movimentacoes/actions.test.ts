...
describe("createTransactionAction", () => {
  it("envia valor normalizado com a chave do formulario e gera chave nova apos sucesso", async () => {
  ...
  it("usa mensagens em pt-BR para tipo, conta e data invalidos", async () => {
  ...
  it("mantem a mesma chave apos falha de rede, para o reenvio ser um replay", async () => {
  ...
  it("mantem a mesma chave apos 500", async () => {
  ...
  it.each([
  ...
  ])("conta indisponivel (%i %s) recarrega o seletor", async (status, code) => {
    ...
    expect(state).toMatchObject({
      status: "error",
      idempotencyKey: KEY,
      message: expect.stringContaining("nao esta mais disponivel"),
    });
    expect(revalidatePath).toHaveBeenCalledWith("/movimentacoes");
  });
  ...
  it("sessao sem token nao chama a API e pede novo login", async () => {
  ...
  it("mapeia errors[].path do 400 para o campo", async () => {
    ...
    expect(state).toMatchObject({
      status: "invalid",
      idempotencyKey: KEY,
      fieldErrors: { occurredOn: ["Informe uma data valida."] },
    });
  });
...
describe("createTransferAction", () => {
  it("resume saida e entrada a partir das duas pernas retornadas", async () => {
